...
inline epoll_type CreateNewEpollFd() { return epoll_create(1); }
...
inline bool EpollAddSocket(epoll_type epoll, socket_t socket, uint32_t events) {
...
inline bool EpollModifySocket(epoll_type epoll, socket_t socket, uint32_t events) {
    struct epoll_event event {};
    event.data.fd = socket;
    event.events  = events;
...
inline bool EpollAddSocket(epoll_type epoll, socket_t socket, uint32_t events, epoll_data_t data) {
    struct epoll_event event {};
    event.data   = data;
    event.events = events;
...
inline bool EpollModifySocket(epoll_type epoll, socket_t socket, uint32_t events, epoll_data_t data) {
    struct epoll_event event {};
    event.data   = data;
    event.events = events;
...
inline bool EpollRemoveSocket(epoll_type epoll, socket_t socket) {
...
// return 0 No Events
// return -1(INVALID_SOCKET) errno
inline int32_t EpollGetEvents(epoll_type epoll, struct epoll_event *events, int32_t events_size, int32_t timeout) {
    assert(events_size > 0);

    return ::epoll_wait(epoll, events, events_size, timeout);
}

inline void CloseEpoll(epoll_type epoll) { ::close(epoll); }
...
public:
    ...
    int32_t GetEvents(epoll_event *events, int32_t events_size, int32_t timeout) {
