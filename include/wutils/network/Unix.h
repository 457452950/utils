...
public:
    ...
    using native_data_type            = decltype(::sockaddr_un::sun_path);
    using native_sockaddr_type        = ::sockaddr_un;
    static constexpr int sockaddr_len = sizeof(native_sockaddr_type);
    using SockAddr                    = std::array<uint8_t, sockaddr_len>;

    static bool GetSockaddr_un(const std::string &path, bool create_file, native_sockaddr_type *sockaddr) {
    ...
    public:
        ...
        bool Assign(const std::string &path) {
            is_valid_ = GetSockaddr_un(path, false, (native_sockaddr_type *)&this->sockaddr_);
            return is_valid_;
        }
