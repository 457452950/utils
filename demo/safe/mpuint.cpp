...
void mpuint::operator/=(unsigned short n) { (void)remainder(n); }

void mpuint::operator%=(unsigned short n) { *this = remainder(n); }
...
void mpuint::Divide(const mpuint &dividend, const mpuint &divisor, mpuint &quotient, mpuint &remainder) {
    if(divisor.IsZero())
        numeric_overflow();
    remainder  = 0;
    quotient   = 0;
